...
var socket = io.connect('http://192.168.10.162:1337');
socket.on('newCommand',function(message){
    var infoCount = parseInt($('.app-information').html());
    if(infoCount == 0){
        $('.app-information').show();
    }
    infoCount = infoCount + 1;
    $('.app-information').html(infoCount);
    $.notify("Job Command Added",{
        className: "info"
    });
});
...
socket.on('newJob',function(message){
    var infoCount = parseInt($('.app-information').html());
    $.notify("Job "+message.jobName+" Added",{
        className: "info"
    });
});
socket.on('commandStartExec',function(message){
    $.notify("Job "+message[0].name+" Start Executed",{
        className: "info"
    });
});
socket.on('commandFailedExec',function(message){
   $.notify("Job "+message[0].name+" Failed Executed",{
        className: "error"
    }); 
});
socket.on('commandSuccessExec',function(message){
    $.notify("Job "+message[0].name+" Success Executed",{
        className: "success"
    });
});